...
test('[req:OBS-2] [req:OBS-6] publisher grava outbox e não bloqueia quando Observer está indisponível', async () => {
...
test('[req:OBS-4] [req:OBS-6] publisher confirma evento e remove somente outbox aceita', async () => {
  const fixture = makeObserverFixture();
...
test('[req:OBS-6] hook publisher encerra com sucesso mesmo sem servidor', () => {
  const fixture = makeObserverFixture();
  try {
    const result = spawnSync(process.execPath, [HOOK], {
      input: JSON.stringify({ cwd: fixture.projectRoot, obsidian_vault_path: fixture.vaultBase }),
      encoding: 'utf8',
      env: { ...process.env, WENDKEEP_OBSERVER_URL: 'http://127.0.0.1:1', WENDKEEP_OBSERVER_TOKEN: 'x' },
    });
