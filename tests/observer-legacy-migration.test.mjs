...
const TOKEN = 'observer-migration-token';
...
test('[req:SQL-OBS-5] startup importa memória Markdown legada do volume para SQLite sem apagar a fonte', async () => {
  ...
  try {
    ...
    try {
      ...
      const document = await (await fetch(`${base}/v1/projects/legacy-project/memory/document?path=${encodeURIComponent(logicalPath)}`, { headers: AUTH_HEADERS })).json();
    ...
    const second = await startObserverServer({ host: '127.0.0.1', port: 0, dataDir, ...observerBootstrap(TOKEN) });
    try {
