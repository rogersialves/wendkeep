...
function seed(db) {
  ...
  return ingestObserverEvents(db, { projectId: 'project-a', events: [
    ...
    event('document.upsert', 'd-old', { logical_path: 'old.md', content: '# Old\n\nold purge marker', revision: 1 }, old),
    event('document.upsert', 'd-new', { logical_path: 'new.md', content: '# New\n\nnew retained marker', revision: 1 }, fresh),
...
test('[req:OBS-SEC-PURGE] purge is transactional, removes derived indexes and emits an idempotent verifiable receipt', () => {
  ...
  try {
    ...
    const searchEngine = observerFts5Support(db);
    assert.ok(db.prepare("SELECT COUNT(*) AS count FROM document_chunks WHERE logical_path = 'old.md'").get().count > 0);
    if (searchEngine.supported) {
      assert.ok(db.prepare("SELECT COUNT(*) AS count FROM evidence_chunks_fts WHERE logical_path = 'old.md'").get().count > 0);
    }
    const options = {
    ...
    if (searchEngine.supported) {
      ...
      assert.equal(searchEngine.engine, 'lexical-fallback');
      assert.equal(db.prepare("SELECT COUNT(*) AS count FROM sqlite_master WHERE type = 'table' AND name = 'evidence_chunks_fts'").get().count, 0);
    }
