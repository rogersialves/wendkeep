...
3. Wira os hooks de sessão do Codex em **`.codex/hooks.json`** — oito dos doze: `brain-inject` + `session-start` no `SessionStart`, `session-ensure` + `change-context` no `UserPromptSubmit`, `session-stop` + `change-nag` no `Stop`, `subagent-stop` no `SubagentStop` e `change-guard` no `PreToolUse` para `Bash`, `exec_command`, `apply_patch` e MCPs mutáveis, sempre na forma `npx wendkeep hook <name>`. O guard aceita payload Codex objeto, string crua e argv; compara a sessão com projeto, raiz Git, remoto, branch e worktree antes da mutação e nega alvo ausente ou divergente. Os outros quatro ficam de fora por falta de payload, ferramenta ou evento equivalente: `change-warn` (*nudge* `PostToolUse` sem `tool_input.file_path` confiável), `plan-capture` (não existe `ExitPlanMode`), `decision-capture` (`AskUserQuestion` é ferramenta só do Claude) e `task-log` (`TaskCompleted` não está no enum de eventos do Codex). No Codex, bloqueios usam `permissionDecision: "deny"`; `ask` não é emitido em `PreToolUse`. O merge é não‑destrutivo, preserva hooks de terceiros e continua migrando `timeout` para `timeoutSec`. **O Codex enumera todo hook como Untrusted e só executa depois que você aprovar o "Hooks need review" no startup — o `init` não consegue pré-aprovar**.
