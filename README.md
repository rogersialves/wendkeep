...
| **Recall e embeddings** | cursor/byte budgets, sidecar lexical/FTS bounded e contrato de plugin local opcional, desligado por padrão | [Plugin opcional de embeddings](https://github.com/rogersialves/wendkeep/blob/main/docs/pt-BR/commands/evidence-embeddings.md) |
...
| **Bridges do ecossistema** | adapters opcionais Spec Kit/Superpowers, baseline canônico, ownership único, drift e prova Git/CI/Envelope com manifest idêntico no índice e worktree | [Bridges do ecossistema](https://github.com/rogersialves/wendkeep/blob/main/docs/pt-BR/commands/ecosystem-bridges.md) |
...
- **O recall padrão continua lexical/FTS e local.** Embeddings exigem um plugin local confiável, fornecido explicitamente, com rede proibida, budgets e fallback lexical; nenhum modelo ou provider viaja no Core.
