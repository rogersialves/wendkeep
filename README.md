...
| **Contexto ativo** | `context switch/status/recover`, transição causal, quarentena e recuperação explícita | [Contexto ativo](https://github.com/rogersialves/wendkeep/blob/main/docs/pt-BR/commands/context.md) |
...
- **Registradores têm escopo.** `git.local-head`, handoffs, vereditos e status de change carregam escopo de projeto, work session, change, branch ou worktree. Duas branches não criam conflito global; somente eventos do mesmo escopo e linhagem causal avançam automaticamente.
- **`EVIDENCE_INDEX.jsonl` é o recall local.** Markdown é dividido por headings e blocos sem depender do Observer. O ranking combina BM25, frase exata, pesos de campo, autoridade, validade, recência limitada e diversidade; `UserPromptSubmit` injeta apenas passagens relevantes com proveniência.
