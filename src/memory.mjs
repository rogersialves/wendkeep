...
import {
  parseSharedMemory, sanitizeMemoryText, renderSharedMemory, validateSharedMemory,
...
export function migrateMemory(vault, {
  ...
  const parsed = parseSharedMemory(legacy);
  const validated = validateSharedMemory(legacy);
  const alreadyV2 = validated.ok && parsed.metadata.schema_version === 2;
  const sourceVersion = parsed.ok && [1, 2].includes(parsed.metadata?.schema_version)
    ? parsed.metadata.schema_version : 0;
  const migrationPlan = planNativeControlPlaneMigration('vault', sourceVersion);
  ...
  if (!apply) return {
    status: 'dry-run', alreadyV2, candidates: candidates.length, backupPath, migration_plan: migrationPlan,
  };
  if (alreadyV2) {
    seedMemoryV2(vault);
    return {
      status: 'unchanged', alreadyV2: true, candidates: 0, backupPath: null,
  ...
  try {
    ...
    return {
      status: 'migrated', alreadyV2: false, candidates: candidates.length, backupPath, validation,
      migration_plan: migrationPlan,
    };
  } catch (error) {
