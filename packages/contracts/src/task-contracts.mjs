...
import { evaluateHostCoverage } from '../../integrations/src/capabilities.mjs';
...
function artifactManifest(changeDir) {
  ...
  return { raw, specs: parsed?.artifacts ?? [], hash: sha256(raw) };
}
...
export function buildTaskContractSnapshot({
  ...
  for (const [name, dependency] of Object.entries({
    activeContextKey, buildEffectiveRequirementPackage, parseTasks, resolveActiveContext,
  })) {
    if (typeof dependency !== 'function') {
      throw Object.assign(new Error(`task contract composition requires ${name}`), {
        code: 'TASK_CONTRACT_COMPOSITION_MISSING',
      });
    }
  }
  ...
  const binding = {
    ...
    tasksSha256: `sha256:${sha256(String(tarefasMd).replace(/\r\n?/g, '\n'))}`,
