...
import { planNativeControlPlaneMigration } from '../../migrations/src/index.mjs';
...
const LEGACY_DRAFT_KEYS = ['schema_version', ...DRAFT_KEYS];

function migrateLegacyReceiptLedger({ store, raw, checkpointRaw, legacyBytesRaw }) {
  if (!raw) return null;
  const parsed = parseLedger(raw);
  const versions = new Set(parsed.records.map((record) => record?.schema_version));
  if (versions.size !== 1 || !versions.has(1)) return null;
  if (checkpointRaw) throw corrupt('Ledger v1 não pode alegar checkpoint v2.');
  const plan = planNativeControlPlaneMigration('ledger', 1);
  const drafts = parsed.records.map((record, index) => {
    assertExactKeys(record, LEGACY_DRAFT_KEYS, `Receipt legado ${index + 1}`);
    return normalizedDraft(Object.fromEntries(DRAFT_KEYS.map((key) => [key, record[key]])));
  });
  const historicalBytes = Buffer.concat([legacyBytesRaw, Buffer.from(raw, 'utf8')]);
  let previousHash = receiptGenesisHash(historicalBytes);
  const records = drafts.map((draft, index) => {
    const record = buildReceiptRecord(draft, { sequence: index + 1, previousHash });
    previousHash = record.receipt_hash;
    return record;
  });
  const migratedRaw = records.map((record) => `${JSON.stringify(record)}\n`).join('');
  atomicWrite(store, store.legacyPath, historicalBytes.toString('utf8'));
  atomicWrite(store, store.ledgerPath, migratedRaw);
  if (records.length) publishCheckpoint(store, migratedRaw, records.at(-1));
  return {
    records,
    raw: migratedRaw,
    legacyBytesRaw: historicalBytes,
    checkpointRaw: records.length ? secureRead(store, store.checkpointPath) : '',
    migrationPlan: plan,
  };
}
...
function readReceiptLedgerUnlocked({ store }) {
  ...
  let legacyBytesRaw = secureRead(store, store.legacyPath, { encoding: null });
  let raw = secureRead(store, store.ledgerPath);
  let checkpointRaw = secureRead(store, store.checkpointPath);
  const migrated = migrateLegacyReceiptLedger({ store, raw, checkpointRaw, legacyBytesRaw });
  let migrationPlan = planNativeControlPlaneMigration('ledger', 2);
  if (migrated) {
    ({ raw, checkpointRaw, legacyBytesRaw, migrationPlan } = migrated);
  }
